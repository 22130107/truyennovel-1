      ...
      <div className="group flex flex-col w-full" style={{"clipPath": clipPathStyle, filter: "drop-shadow(2px 0 0 var(--color-pink)) drop-shadow(0 2px 0 var(--color-pink)) drop-shadow(-2px 0 0 var(--color-pink)) drop-shadow(0 -2px 0 var(--color-pink))"}}>
        ...
        <div className="items-start flex bg-black/80 backdrop-blur-md p-2 min-h-[100px]" style={{ color: "var(--color-dura-1)" }}>
          <div className="items-center flex justify-center w-12 h-12 mt-[8px] mr-[8px]">
            <span className="bg-clip-text block italic font-extrabold text-transparent text-[48px] leading-[48px]" style={{ backgroundImage: "linear-gradient(var(--color-dura-4), var(--color-dura-5))" }}>{rank}</span>
          </div>
          <div className="flex flex-col justify-start gap-[4px] flex-1">
            <p className="flow-root font-bold overflow-hidden line-clamp-2 h-[40px] leading-[20px]" style={{ color: "var(--color-dura-1)" }}>{title}</p>
            <p className="flow-root truncate text-[12px] leading-[16px]" style={{ color: "var(--color-dura-2)" }}>Dịch giả:  {translator}</p>
            <p className="font-semibold text-[12px] leading-[16px]" style={{ color: "var(--color-dura-3)" }}>{date} - {chapters} ch</p>
          </div>
        </div>
      </div>
    </Link>
